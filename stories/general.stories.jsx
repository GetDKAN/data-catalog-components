...
storiesOf("General", module)
  ...
  .add("Menu (external links)", () => (
    <Menu
      items={links.footer1}
      title={text("Title", "External links")}
      menuId="sampleNav"
    />
  ))
  .add("Menu (internal links", () => (
    <Menu
      items={links.main}
      title={text("Title", "Internal links")}
      menuId="sampleNav"
    />
  ))
  ...
  .add("Show More Container", () => {
    ...
    return (
      <ShowMoreContainer
        ...
        items={array("Show More Items", itemsArray).map((item) => (
          <li key={item}>{item}</li>
        ))}
  ...
  .add("FontAwesome Brand Icon", () => (
    <span>
      <FontAwesomeIcon icon={["fab", "facebook"]} color="blue" size="4x" />
      <FontAwesomeIcon icon={["fab", "twitter"]} color="aqua" size="4x" />
      <FontAwesomeIcon icon={["fab", "github"]} color="purple" size="4x" />
    </span>
  ))
  .add("Footer", () => <Footer links={links} customClasses="container-fluid" />)
  .add("API Documentation", () => (
    <ApiDocs endpoint="https://petstore.swagger.io/v2/swagger.json" />
  ))
  .add("Announcement", () => (
    <Announcement
  ...
  .add("Publisher List", () => <PublisherList items={orgs} className="test" />);
